                           ...
                           const bool uniq_build_tbl_keys)
{
    ...
    RMM_CALL(RMM_ALLOC(&devStates, num_states * sizeof(curandState), 0));
    ...
    RMM_CALL(RMM_ALLOC(&build_tbl_sorted, build_tbl_size * sizeof(key_type), 0));

    size_type lottery_size = rand_max < std::numeric_limits<key_type>::max() - 1 ? rand_max + 1 : rand_max;
    key_type* lottery;

    RMM_CALL(RMM_ALLOC(&lottery, lottery_size * sizeof(key_type), 0));

    if (uniq_build_tbl_keys) {
        thrust::sequence(rmm::exec_policy(0)->on(0), lottery, lottery + lottery_size, 0);
    }
    ...
    thrust::sort(rmm::exec_policy(0)->on(0), build_tbl_sorted, build_tbl_sorted + build_tbl_size);
    ...
    key_type *lottery_end = thrust::set_difference(
        rmm::exec_policy(0)->on(0), first_lottery_elem, last_lottery_elem,
