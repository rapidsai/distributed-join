 ...
 *
 * This function is useful, for example, for calculating the size on root and allocating receive buffer on workers when
 * distributing tables from root node to worker nodes. This function mainly solves the problem when the number of ranks
 * does not divide the number of rows in the global table.
 *
 * @param[in] global_table_size     Number of rows in the global table.
 * @param[in] mpi_rank              Target rank for which this function will calculate the local table size.
 * @param[in] mpi_size              Total number of MPI ranks.
 *
 * @returns                         Number of rows in the local table of rank 'mpi_rank'.
 */
inline gdf_size_type get_local_table_size(gdf_size_type global_table_size, int mpi_rank, int mpi_size)
{
    gdf_size_type local_table_size = global_table_size / mpi_size;
 ...
 * @param[out] local_col      The preallocated local table column. Significant on all ranks.
 * @param[in]  communicator   An instance of `Communicator` used for communication.
 */
void distribute_cols(
                     const gdf_column *global_col,
                     gdf_column *local_col,
                     Communicator *communicator)
{
    ...
    gdf_size_type dtype_size = gdf_dtype_size(local_col->dtype);

    if (mpi_rank == 0) {
        // master node

        gdf_size_type global_size = global_col->size;
        requests.resize(mpi_size);

        // Send global_col to each slave node

        for (int irank = 1; irank < mpi_size; irank++) {
            gdf_size_type start_idx = min(irank, (int)(global_size % mpi_size)) + (global_size / mpi_size) * irank;
            gdf_size_type irank_size = get_local_table_size(global_size, irank, mpi_size);
            void *start_addr = (void *)((char *)(global_col->data) + start_idx * dtype_size);

            requests[irank] = communicator->send(start_addr, irank_size, dtype_size, irank, distribute_col_tag);
        }

        // Fill master node's local_col

        gdf_size_type rank0_size = get_local_table_size(global_size, 0, mpi_size);

        CUDA_RT_CALL(cudaMemcpy(local_col->data, global_col->data, rank0_size * dtype_size, cudaMemcpyDeviceToDevice));
        ...
        comm_handle_t request = communicator->recv(local_col->data, local_col->size, dtype_size, 0, distribute_col_tag);
 ...
 * @param[in] communicator     An instance of `Communicator` used for communication.
 *
 * @returns                    The local table on each rank. Note the user of this function is responsible for freeing
 *                             the returned table as well as the device buffer it contains. See 'free_table'.
 */
std::vector<gdf_column *> distribute_table(
                                           std::vector<gdf_column *> global_table,
                                           Communicator *communicator)
{
    /* Get MPI information */
    ...
    int mpi_rank {communicator->mpi_rank};
    int mpi_size {communicator->mpi_size};
    MPI_Datatype mpi_gdf_size_type = mpi_dtype_from_c_type<gdf_size_type>();

    /* Broadcast global table size */

    gdf_size_type global_table_size {-1};

    if (mpi_rank == 0) {
        global_table_size = global_table[0]->size;
    }

    MPI_Bcast(&global_table_size, 1, mpi_gdf_size_type, 0, MPI_COMM_WORLD);
    ...
    if (mpi_rank == 0) {
        table_ncols = global_table.size();
    }

    MPI_Bcast(&table_ncols, 1, MPI_INT, 0, MPI_COMM_WORLD);

    /* Broadcast column datatype */

    std::vector<gdf_dtype> columns_dtype(table_ncols);

    // TODO: use cudf::column_dtypes?

    for (int icol = 0; icol < table_ncols; icol++) {

        if (mpi_rank == 0)
            columns_dtype[icol] = global_table[icol]->dtype;

        MPI_Bcast(&columns_dtype[icol], 1, MPI_INT, 0, MPI_COMM_WORLD);
    }

    /* Allocate local tables across nodes */

    gdf_size_type local_table_size = get_local_table_size(global_table_size, mpi_rank, mpi_size);

    std::vector<gdf_column *> local_table(table_ncols, nullptr);

    for (int icol = 0; icol < table_ncols; icol++) {
        void *data;
        size_t alloc_size = local_table_size * gdf_dtype_size(columns_dtype[icol]);

        RMM_CALL(RMM_ALLOC(&data, alloc_size, 0));

        local_table[icol] = new gdf_column;

        GDF_CALL(gdf_column_view(local_table[icol], data, nullptr, local_table_size, columns_dtype[icol]));
    }

    /* Send table from root to all nodes */

    for (int icol = 0; icol < table_ncols; icol++) {
        gdf_column *global_col {nullptr};

        if (mpi_rank == 0)
            global_col = global_table[icol];

        distribute_cols(global_col, local_table[icol], communicator);
    }
...
/**
 * Helper function for hashing a table into partitions.
 *
 * @param[in] table               Table to be hashed.
 * @param[out] hashed_table       The reordering of the input 'table' into multiple partitions according to the hash
 *                                function. This argument does not need to be preallocated but the caller is responsible
 *                                for freeing the returned table as well as the device buffer it contains. See
 *                                'free_table'.
 * @param[out] partition_offsets  Start indices of each partition in 'hashed_table'. This argument does not need to be
 *                                preallocated but the caller is responsible for freeing the returned device buffer
 *                                using RMM_FREE. This array should have length npartition + 1.
 * @param[in] npartition          The number of partitions. If not specified or specified as 0, the number of mpi ranks
 *                                will be used.
 */
void hash_table(
                std::vector<gdf_column *> &table,
                ...
                int npartition=0)
{
    ...
    int columns_to_hash[] = {0};

    hashed_table.resize(table.size(), nullptr);

    for (int icol = 0; icol < table.size(); icol++) {
        gdf_size_type dtype_size = gdf_dtype_size(table[icol]->dtype);

        void *data;
        size_t alloc_size = table[icol]->size * dtype_size;

        RMM_CALL(RMM_ALLOC(&data, alloc_size, 0));

        hashed_table[icol] = new gdf_column;

        GDF_CALL(gdf_column_view(hashed_table[icol], data, nullptr, table[icol]->size, table[icol]->dtype));
    }

    RMM_CALL(RMM_ALLOC(&partition_offsets, (npartition + 1) * sizeof(int), 0));

    GDF_CALL(gdf_hash_partition(
        table.size(), table.data(), columns_to_hash, 1, npartition, hashed_table.data(),
        partition_offsets, GDF_HASH_IDENTITY
    ));

    CUDA_RT_CALL(cudaMemcpy(&partition_offsets[npartition], &(table[0]->size), sizeof(int), cudaMemcpyHostToDevice));
}
